...
const createTranslation = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ Message: 'Translation saved!', translationId: obj.id });
  } catch (error) {
...
const readTranslationsByUid = async (req, res) => {
  try {
    ...
    if (!translations) {
      return res
        .status(404)
        .json({ error: `Translations could not be fetched for uid ${uid}` });
    }
...
const clearTranslationHistoryByUid = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ Message: 'Translation history cleared successfully' });
  } catch (error) {
    console.log(`Error occurred: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
