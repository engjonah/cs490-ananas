...
function generateMockToken(id = 'default') {
  return jwt.sign({ uid: `mockUserId${id}` }, process.env.JWT_TOKEN_KEY, {
    expiresIn: '1h',
  });
}
...
describe('/api/translate mocked API requests', () => {
  ...
  it('should return 429 status code with error for rate limit exceeded', async () => {
    nock(baseURL).post(endpoint).reply(429);
    return await request(app)
      .post('/api/translate')
      .send({
        inputLang: 'Python',
  ...
  it('should return 503 status code with error for API connection error', async () => {
    nock(baseURL).post(endpoint).reply(503);
    return await request(app)
      .post('/api/translate')
      .send({
        inputLang: 'Python',
  ...
  it('should return 500 status code with error for unknown error occurred', async () => {
    nock(baseURL).post(endpoint).reply(500);
    return await request(app)
      .post('/api/translate')
      .send({
        inputLang: 'Python',
...
describe('Caching Unit Tests', () => {
  ...
  const inputs = [
    { inputLang: 'Python', outputLang: 'Java', inputCode: 'MockCode' },
    { inputLang: 'Python', outputLang: 'C++', inputCode: 'MockCode' },
    { inputLang: 'Python', outputLang: 'Ruby', inputCode: 'MockCode' },
  ];
  const cacheContents = [
    {
      inputLang: 'Python',
