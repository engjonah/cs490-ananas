...
describe('CodeBox Component Functionality', () => {
  test('CodeBox code box starts rending', async () => {
    render(<CodeBox />);
    expect(
      screen.getByText('Loading your pudgy penguins...')
    ).toBeInTheDocument();
  });
  ...
  test('Download button downloads code file', async () => {
    ...
    const file = new Blob(["const example = 'Hello!';"], {
      type: 'text/plain',
    });
  ...
  test('toast success notif', async () => {
    ...
    render(
      <CodeBox
        setOutputLoading={jest.fn()}
        setOutputCode={jest.fn()}
        user={{ uid: 'mockuid' }}
        outputLang={1}
      />
    );
  ...
  test('toast 503 error notif', async () => {
    ...
    global.fetch = jest.fn(() =>
      Promise.resolve(new Response(JSON.stringify({}), { status: 503 }))
    );
